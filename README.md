...
Run the first cell and upload Batting.csv, Pitching.csv, Fielding.csv, People.csv, and Salaries.csv from the Lahman Database when prompted.

Run all cells in order — later cells depend on functions and dataframes defined earlier (battingStats, pitchingStats, hittingStats, the Formula OVR functions, and the trained RandomForest models).

Use the dropdown widgets that appear under each @title cell to select a year (and team, for hitters) and generate ratings, projections, or salary tables.
